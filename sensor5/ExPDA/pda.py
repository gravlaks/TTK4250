...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def predict(self, filter_state: ET, Ts: float) -> ET:
        ...
        return self.state_filter.predict(filter_state, Ts) #Done
    ...
    def loglikelihood_ratios(
        ...
        *,
        ...
        #This is implementing the log of 7.3.3 in the compendium.

        log_PD = np.log(self.PD)
        log_PND = np.log(1 - self.PD)  # P_ND = 1 - P_D
        log_clutter = np.log(self.clutter_intensity) #This is Lambda

        # allocate
        ll = np.empty(Z.shape[0] + 1)

        # calculate log likelihood ratios
        ll[0] = log_clutter*log_PND # Done: missed detection
        ll[1:] = [log_PD + self.state_filter.loglikelihood(z, elem, sensor_state=sensor_state) for z in Z]# Done: some for loop over elements of Z using self.state_filter.loglikelihood
    ...
    def association_probabilities(
        ...
        *,
        ...
        #7.3.3 only gave us scalar*beta. We have to normalize
        # probabilities
        beta = np.exp(lls)/np.sum(beta) #Sum has to be equal to 1.  #Done
    ...
    def conditional_update(
        ...
        *,
        ...
        conditional_update = []
        conditional_update.append( #Turns out that filter state is x^k|k-1
            filter_state # Done: missed detection
        )
    ...
    def reduce_mixture(
        ...
        return  self.state_filter.reduce_mixture(mixture_filter_state)# Done: utilize self.state_filter to keep this working for both EKF and IMM
    ...
    def update(
        ...
        *,
        ...
        gated = self.gate(Z, filter_state, sensor_state=sensor_state)# Done
        Zg = Z[gated] #Remove false lists
    ...
    def step(
        ...
        *,
        ...
        filter_state_predicted = self.predict(filter_state, Ts) # Done
        filter_state_updated = self.update(Z, filter_state_predicted, sensor_state=sensor_state)# done
    ...
    def estimate(self, filter_state: ET) -> GaussParams:
        ...
        return self.state_filter.estimate(filter_state) #Done
